...
public:
  ...
  double *mass;
  double *massset;
  double *xp, *yp, *zp, *vp;         // position in Cartesian coord.
  BinarySystem(int np);
  ~BinarySystem();
public:
      ...
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        for (int ip=0; ip< psys->np; ++ip){
          ...
          if (r_s!=0){
            ...
            mass_sink = cons(IDN,k,j,i) * voluume;
          }
  ...
  if (psys->np > 0) {
    ...
    if(std::strcmp(COORDINATE_SYSTEM, "spherical_polar") == 0){
      for (int ip=0; ip< psys->np; ++ip){
        ...
        //   EccA = compute_newton_raphson(10,2 * PI * time / period,eccent); // eccentric anomaly
        //   TrA = 2.0 * atan(sqrt((1.0 - eccent) / (1.0 + eccent)) * tan(EccA / 2.0)); // true anomaly
        //   // TrA = 2.0 * atan2(sqrt(1.0 - eccent * eccent) * sin(E) / (1.0 + eccent * cos(E)), cos(E) / (1.0 + eccent)); // true anomaly
        //   xpp = 0.5 * a_bin * (cos(TrA) - eccent) - psys->xp[ip];
        //   ypp = 0.5 * a_bin * sqrt(1 - eccent * eccent) * sin(TrA);
        //   zpp = 0.0;
        // } else if(ip==1){
        ...
        for (int k=pmb->ks; k<=pmb->ke; ++k) {
          ...
          for (int j=pmb->js; j<=pmb->je; ++j) {
            ...
            for (int i=pmb->is; i<=pmb->ie; ++i) {
              ...
              /*ecc = eccentricity(time);
              if(time!=0){
                std::cout << "the time in force is "<<pmb->pmy_mesh->time<<std::endl;
                std::cout << "the eccentricity in force is "<<ecc<<std::endl;
              }*/
              /*if (std::abs(xcar-xpp)<0.001){
                std::cout << "the mom1 is "<<pmb->phydro->u(IM1,k,j,i)<<std::endl;
              }*/
        ...
        if (iout==6&&ip==0) return f_xpp;
        if (iout==7&&ip==0) return f_ypp;
        if (iout==8&&ip==0) return f_zpp;
        if (iout==9&&ip==1) return f_xpp;
        if (iout==10&&ip==1) return f_ypp;
        if (iout==11&&ip==1) return f_zpp;
        if (iout==12&&ip==0) return torque;
        if (iout==13&&ip==1) return torque;
        if (rank == 0) {
...
void BinarySystem::Position(double time, int obj){
  ...
  double period = 2.0 * PI * sqrt(pow(a_bin * 0.5, 3) / (2 * 1.5e-5)); // period 
