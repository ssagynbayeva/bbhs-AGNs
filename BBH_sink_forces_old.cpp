      ...
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        ...
        for (int ip=0; ip< psys->np; ++ip){
          ...
          if (r_s!=0){
            ...
            f_acc_x += f_acc_r * sin(pco->x2v(j)) * cos(pco->x3v(k)) + f_acc_theta * cos(pco->x2v(j)) * cos(pco->x3v(k)) - f_acc_phi * sin(pco->x3v(k));
            f_acc_y += f_acc_r * sin(pco->x2v(j)) * sin(pco->x3v(k)) + f_acc_theta * cos(pco->x2v(j)) * sin(pco->x3v(k)) + f_acc_phi * cos(pco->x3v(k));
            f_acc_z += f_acc_r * cos(pco->x2v(j)) - f_acc_theta * sin(pco->x2v(j));
  ...
  if (psys->np > 0) {
    ...
    if(std::strcmp(COORDINATE_SYSTEM, "spherical_polar") == 0){
      for (int ip=0; ip< psys->np; ++ip){
        ...
        for (int k=pmb->ks; k<=pmb->ke; ++k) {
          ...
          for (int j=pmb->js; j<=pmb->je; ++j) {
            ...
            for (int i=pmb->is; i<=pmb->ie; ++i) {
              ...
              f_x = f_xpp + f_acc_x; // this is the total force applied on a black hole
              f_y = f_ypp + f_acc_y;
              f_z = f_zpp + f_acc_z;
        ...
        if (iout==34&&ip==0) return f_acc_x;
        if (iout==35&&ip==1) return f_acc_x;
        if (iout==36&&ip==0) return f_acc_y;
        if (iout==37&&ip==1) return f_acc_y;
        if (iout==38&&ip==0) return f_acc_z;
        if (iout==39&&ip==1) return f_acc_z;
